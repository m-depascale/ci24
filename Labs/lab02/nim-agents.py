...
def generate_move(s):
    ...
    print(f'\nROWS NON NULL: {rows} FOR STATE {s}')
    return Nimply(r, o)
...
class Agent_II():
    ...
    def _game(self, nim, player, population, states_0, states_1, moves_0, moves_1):
        while nim:
            move = None
    ...
    def training(self):
        ...
        for generation in range(GENERATIONS):
            ...
            for i in range(10):
                ...
                s0, m0, s1, m1 = self._game(game, p, pop, s0, s1, m0, m1 )
                print()
                #print(f'Game {i + 1} won by player {player}')

                offsprings.append(Genotype(s0, m0))
                offsprings.append(Genotype(s1, m1))
